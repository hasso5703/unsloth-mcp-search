...
- `url`   → fetches that URL with SSRF protection, IP pinning, redirect handling,
            binary/PDF detection, and converts HTML to clean Markdown.

Stripped of the Studio-specific session/sandbox/code-execution machinery, and of
the `website_policy` (allow/block domain lists) and `cancel_event` plumbing that
only make sense inside a Studio chat run. The overall fetch deadline is kept.
"""
...
_GITHUB_NON_OWNER_SEGMENTS = frozenset(
    {
        "about", "apps", "codespaces", "collections", "contact",
        "customer-stories", "dashboard", "discussions", "enterprise",
        "explore", "features", "issues", "join", "login", "marketplace",
        "new", "notifications", "organizations", "orgs", "pricing", "pulls",
        "search", "security", "settings", "signup", "site", "sponsors",
...
_HTML_LEADING_TAGS = (
    ...
    "figure", "form", "ul", "ol", "dl", "pre", "blockquote",
)
_HTML_LEADING_RE = re.compile(
    r"<(?:!doctype\s+html|/?(?:" + "|".join(_HTML_LEADING_TAGS) + r")\b)"
)
# Stricter than _HTML_LEADING_RE: only a real document opener, never a block tag
# a Markdown file can open with. Used on the raw GitHub README body so a Markdown
# README starting with an HTML block is not run through html_to_markdown, which
# would collapse its headings, lists and fenced code onto one line.
...
def _extract_pdf_text(data: bytes) -> str:
    ...
    Upstream routes this through Studio's RAG parser (pymupdf); a standalone
    server uses `pypdf`, which is pure Python and text-only.
    """
    import io
    ...
    try:
        ...
        return "(PDF text extraction requires the optional 'pypdf' dependency)"
    ...
    length = 0
    text_limited = False
    for number, page in enumerate(reader.pages[:_MAX_WEB_PDF_PAGES], start=1):
        try:
...
def _check_url_access(url: str) -> tuple[bool, str, str]:
    """Scheme/host gate standing in for Studio's website-policy check.

    Returns ``(allowed, reason_or_empty, hostname)``. Studio filters against
    per-run allow/block domain lists here; a standalone server only enforces
    the scheme and a parseable host/port.
    """
    from urllib.parse import urlparse
    ...
    try:
        ...
        return False, f"Blocked: malformed URL ({e}).", ""
    if scheme not in ("http", "https"):
        return False, f"Blocked: only http/https URLs are allowed (got {scheme!r}).", ""
    if not hostname:
        return False, "Blocked: URL is missing a hostname.", ""
...
def _fetch_budget_exceeded(deadline: float | None) -> str | None:
...
def _fetch_hop_timeout(timeout, deadline: float | None):
...
def _resolve_with_budget(hostname: str, port: int, deadline: float | None):
    ...
    could run past the budget. Resolve on a daemon thread and poll the budget so
    the fetch aborts on time; the abandoned lookup is discarded. With no deadline
    this is a plain synchronous call, so opt-out callers keep the old cost.
    """
    budget_error = _fetch_budget_exceeded(deadline)
...
def _read_capped_body(resp, max_bytes: int, timeout, deadline: float | None):
...
def _fetch_url_raw(
    ...
    "Blocked:" / "Failed to fetch URL:" wording), else ``None``. Blocks
    private/loopback/link-local targets and caps the download size. No input
    reaches the caller as an exception: the URL is model-supplied, so every
    malformed form resolves to one of these strings.

    ``deadline`` is an optional ``time.monotonic`` cutoff for the whole fetch
    (redirect hops and body read included); it defaults off so callers keep the
    old behavior.
    """
    from urllib.parse import urlparse

    # Before the access gate: it requires an http(s) scheme, so a bare host
    # would be refused there and never reach the fetch.
    url = _normalize_url_scheme(url)
    allowed, reason, canonical_host = _check_url_access(url)
    if not allowed:
        return reason, "", ""

    # _check_url_access already parsed this and read .port, so this cannot raise.
    parsed = urlparse(url)
    port = parsed.port or (443 if parsed.scheme == "https" else 80)
    ok, reason, pinned_ip = _resolve_with_budget(canonical_host, port, deadline)
    if not ok:
        return reason, "", ""
    ...
    try:
        ...
        for _hop in range(5):
            ...
            try:
                ...
                allowed, gate_reason, redirect_host = _check_url_access(current_url)
                if not allowed:
                    return gate_reason, "", ""
                rp = urlparse(current_url)
                rp_port = rp.port or (443 if rp.scheme == "https" else 80)
                ok2, reason2, pinned_ip = _resolve_with_budget(redirect_host, rp_port, deadline)
                if not ok2:
                    return reason2, "", ""
            ...
            body_error, raw_bytes = _read_capped_body(resp, read_limit, timeout, deadline)
            if body_error is not None:
                return body_error, "", ""

            # A missing or wrong PDF MIME type is common: once the initial
            # text-sized read identifies PDF magic, finish the bounded download
            # to reach the EOF xref.
        ...
        # Reject known-binary MIME types before decoding. Binary is returned as
        # the error string so the caller surfaces the placeholder, not
        # replacement chars.
...
def fetch_page_text(url: str, max_chars: int = _MAX_PAGE_CHARS, timeout: int = 30) -> str:
    ...
    # Before the access gate (needs a scheme) and the README routing (reads host/path).
    url = _normalize_url_scheme(url)
    allowed, reason, _hostname = _check_url_access(url)
    if not allowed:
        return reason
    ...
    if readme_api_url:
        ...
        # back to the HTML page fetch. A 200 body is authoritative even when it
        # is HTML (a .html README): convert it rather than falling back to the
        # repo page's UI chrome, keeping the raw body if extraction yields nothing.
        if err is None and body.strip():
            readme_body = body
            # The raw file is almost always Markdown. Only a real HTML document
            # (a .html README) is converted; a Markdown README that merely opens
            # with a block tag is kept as-is (see _HTML_DOCUMENT_RE).
    ...
    err, body, content_type = _fetch_url_raw(url, timeout=timeout, deadline=deadline)
    if err is not None:
        return err
